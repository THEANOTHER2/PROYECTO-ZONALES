  ...
  <div class="container">
    ...
    <table class="table fil" id="table">
      <thead class="text-center">
        <tr>
          <th colspan="9">Calificaciones de Danza Popular</th>
        </tr>
        <tr>
          <th>Regionales</th>
          <th>Jurados</th>
      ...
      <tbody>
        <?php
            ...
            $query = $con->query("SELECT id, id_jurado, jurado.nombre AS 'jurado',regional.nombre AS 'regional',calidad_interpretativa,composicion_coreografia,esamble_y_sincronizacion,vestuario,investigacion,total FROM danza_moderna,jurado,regional WHERE danza_moderna.id_regional=regional.id_regional AND danza_moderna.id_jurado=jurado.n°_documento ORDER BY total DESC");
            ...
            if(isset($res)){
              foreach($res as $key=>$value){
                if($value->id_jurado == $jurado){
                  echo '<tr><td>'.$value->regional.'</td><td>'.$value->jurado.'</td><td>'.$value->calidad_interpretativa.'</td><td>'.$value->composicion_coreografia.'</td><td>'.$value->esamble_y_sincronizacion.'</td><td>'.$value->vestuario.'</td><td>'.$value->investigacion.'</td><td>'.$value->total.'</td><td><a class="btn btn-danger" href="editDM.php?id='.$value->id.'">Editar</a></td></tr>';
                }else{
